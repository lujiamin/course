...
##### chapter 7数据预测与估算算法
- 产生式模型和判别式模型
  - 产生式模型估算的是X和Y的联合概率分布P(x,y)
  - 判别式模型估算的是条件概率分布P(y|x)
- 基于最大似然估计的预测
  - 给定样本数组和固定模型，但是模型参数未知，确定未知参数使得模型在已知样本中的概率最大
- 基于线性回归的估算
  - 单变量的线性回归：Y=α+βx+ε，ε是误差项
  - 最小二乘法，最小平方法
  ```
  二维坐标系中(10,20),(11,23),(12,25),(13,27),(14,26)
  找出y=ax+b
  带入点，20=10a+b,23=11a+b,25=12a+b,27=13a+b,26=14a+b
  由最小二乘法，S(a,b) = (20-10a-b)^2+(23-11a-b)^2+(25-12a-b)^2+(27-13a-b)^2+(26-14a-b)^2
  S(a,b)对a和b求偏导，令成0
  求得a和b，y=24.1708+0.0243x
  ```
- 基于最大期望算法分析
  - 在已知变量的情况下，通过迭代的方式估计出未知变量
  - 利用α来估计β，再利用β重新估计α，持续这个过程直到收敛为止
- 基于隐马尔科夫模型预测
  - 状态被隐藏，但是与状态相关的某些变量则是已知的
- 基于条件随机场的序列预测
  - 随机场
  - 马尔科夫随机场
  - 马尔科夫性质
...
- 基于ID3算法的决策分析
  - 信息量：I=--log2(m)，m表示信息值
  - 信息熵：Entropy(x) = -∑p·log2(1/p)
  - 信息增益
- 基于C4.5算法的分类决策树
- 基于分类回归树的决策划分
- 基于随机森林的决策分类 
#### chapter 9 数据关联规则分析算法
- 基于Apriori算法的关联项分析
  - 超市的货架摆放问题
  - 算法原理
    - 1)对数据进行频率次数统计
    - 2)构建候选集C1，计算其支持度，即数据出现频率次数与总数的比
    - 3)对候选集的支持度进行筛选，形成频繁项集L1
    - 4)对频繁项集进行连接生成候选项集C2，重复上述步骤
- 基于FP-Growth算法的关联性分析
  - 优点是只需要对交易数据表进行两次扫描，通过构建FP-树对原始数据进行压缩
- 基于Eclat算法的频繁项集挖掘
  - 利用倒排表的思想进行数据统计      
#### chapter 10 数据推荐算法
- 基于Item-Based协同过滤推荐
  - 利用商品本身属性之间的关联分析，计算物品之间的相似度
  - Slope One
- 基于User-Based系统过滤推荐      
  - 发掘用户的特征，具有相同特征的用户划为一类
- 基于潜在因子算法的推荐
  - 通过用户的历史行为挖掘用户本身的特征，以及分析现有商品的本质特征进行的推荐
  - 对于新加入的用户，缺少历史行为，导致最初的推荐内容相关性较低  
